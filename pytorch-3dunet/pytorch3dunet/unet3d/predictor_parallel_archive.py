...
class StandardPredictor(_AbstractPredictor):
    ...
    def predict(self, prev_iter=(None, None, None), is_last=False):
        ...
        prediction_maps, normalization_masks = self._allocate_prediction_maps(prediction_maps_shape,
                                                                            output_heads) # Could skip this for the first iter
        if not is_last:
            ...
            with torch.no_grad():
                for batch, indices in self.loader:
                    # print(f"Loading {batch[0,0,0,0,0:5]}")
                    ...
                    # predictions = predictions[predictions > pred_threshold]
                    ...
                    # 3) for each head, kick off an async D2H into **pinned** buffers
                    cpu_staging = []
                    with torch.cuda.stream(d2h_stream):
                        for pred in predictions:
                            pred = pred.to(torch.float32)            # optional: dtype you want in CPU
                            cpu_buf = torch.empty_like(pred, device="cpu", pin_memory=True)
                            cpu_buf.copy_(pred, non_blocking=True)   # async D2H into pinned
                            cpu_staging.append(cpu_buf)

                    # 4) while D2H for *this* iter is happening, process *previous* iter on CPU
                    # print(f"HMMM {prev_cpu}")

        # prev_cpu, prev_idx, prev_out_file = prev_iter
        prev_cpu, prev_idx, prev_out_file, prev_shape, prev_halo, prev_dataset = prev_iter
        
        if prev_cpu is not None:
            print("Processing previous batch")
            h5_output_file = h5py.File(prev_out_file, 'w')

            # no GPU sync here; we already waited for d2h_stream last loop
            for cpu_pred, prediction_map, normalization_mask in zip(prev_cpu, prediction_maps, normalization_masks):
                arr = cpu_pred.numpy()                   # zero-copy from pinned tensor
                # print(f"Saving {arr[0,0,0,0,0:5]}")

                # if check("arr", arr): return
                # vectorize if you can; otherwise keep your per-sample loop:
                for pred, index in zip(arr, prev_idx):
                    ch = slice(0, out_channels) if prediction_channel is None else slice(0, 1)
                    index = (ch,) + tuple(index)
                    if prediction_channel is not None:
                        pred = np.expand_dims(pred[prediction_channel], 0)
                    u_prediction, u_index = remove_halo(pred, index, volume_shape, patch_halo)
                    # if check("u_prediction", u_prediction): return
                    prediction_map[u_index] += u_prediction
                    normalization_mask[u_index] += 1
            
            self._save_results(prediction_maps, normalization_masks, output_heads, h5_output_file, self.loader.dataset)
            logger.info('Finished predictions. Saving results...')
            h5_output_file.close()


        # 5) handoff: ensure this iter’s D2H finished before using next loop
        if not is_last:
            # compute_stream.wait_stream(d2h_stream)
            # print(f"Staging {cpu_staging[0][0,0,0,0,0:5]}")
            # prev_cpu = cpu_staging
            # # print(f"HMMM {prev_cpu}")
            # prev_idx = indices
            # prev_out_file = self.output_file
            # # prof.step()
            compute_stream.wait_stream(d2h_stream)
            prev_cpu       = cpu_staging
            prev_idx       = indices
            prev_out_file  = self.output_file
            prev_volume_shape = self._volume_shape(self.loader.dataset)
            prev_patch_halo   = patch_halo
            prev_dataset_obj  = self.loader.dataset
            return (prev_cpu, prev_idx, prev_out_file, prev_volume_shape, prev_patch_halo, prev_dataset_obj)

                
                # Where all the post processing should happen if we want to speed stuff up 
                    
            # save results to
            # close the output H5 file
            # print((prev_cpu[0,0,0,0,0:5], prev_idx, prev_out_file))
            # handoff at the end (unchanged logic, just more fields)
            # return (prev_cpu, prev_idx, prev_out_file, prev_volume_shape, prev_patch_halo, prev_dataset_obj)

            # return (prev_cpu, prev_idx, prev_out_file)
