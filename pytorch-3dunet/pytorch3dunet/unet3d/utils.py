...
def expand_as_one_hot_w_unlabeled(input, C, unlabeled_index):
    """
    Converts NxDxHxW label image to NxCxDxHxW, where each label gets converted to its corresponding one-hot vector
    :param input: 4D input image (NxDxHxW)
    :param C: number of channels/labels
    ...
    # create ignore_index mask for the result
    # mask = input.expand(shape) == unlabeled_index
    mask = input[0][0] == unlabeled_index
    # clone the src tensor and zero out ignore_index in the input
    # input = input.clone()
    input[input == unlabeled_index] = 0
    # scatter to get the one-hot tensor
    one_hot = torch.zeros(shape, dtype=torch.int8, device=input.device).scatter_(1, input, 1) 


    # shape = [-1] + list(result.shape[2:])

    # torch.repeat(torch.sum(result, axis=(2,3,4)) > 0)
    # used_labels = torch.transpose(torch.sum(result, axis=(2,3,4)).nonzero(), 0, 1)[1]
    used_labels = (torch.sum(one_hot, axis=(2,3,4), dtype=one_hot.dtype) > 0)[0]
    # used_labels[0] = False # Don't put a negative 1 in the background
    used_labels[120] = False # TODO: what about gila and granules
    used_labels[121] = False # Maybe they just wouldn't ever be labeled?
    used_labels = used_labels.reshape(-1,1,1,1).expand(shape) # Shouldn't take up any memory
    mask = mask.expand(shape)
    mask = used_labels & mask
    # result[mask * used_labels] = -1
    one_hot[mask] = -1

    # with h5py.File("/home/brian/notebooks/brian/CellAutoLab_FullyCollapsed/encodeTest.h5", 'w') as f:
    #     f.create_dataset('label', data=input_save.cpu())
    #     f.create_dataset('encoded', data=one_hot.cpu())
