...
from pytorch3dunet.unet3d.utils import get_logger, expand_as_one_hot, expand_as_one_hot_w_unlabeled, plot_segm, convert_to_numpy
...
class PixelWiseMeanIoU:
    """
    Computes IoU for each class separately, weighted by pixel, and then averages over all classes.
    Can be set to instead compute MeanIOU over all pixels with nonzero weight if weight_equal parameter is true
    """
    ...
    def __init__(self, skip_channels=(), ignore_index=None, **kwargs):
        ...
        self.ignore_index = ignore_index
        self.skip_channels = skip_channels
        self.weight_equal = kwargs.get("weight_equal", False)
    ...
    def __call__(self, input, target, weights):
        ...
        if target.dim() == 4:
            target = expand_as_one_hot_w_unlabeled(target, C=n_classes, unlabeled_index=n_classes)
        ...
        for batch in range(input.shape[0]): 
            ...
            assert per_channel_iou, "All channels were ignored from the computation"
            mean_iou = torch.mean(torch.tensor(per_channel_iou))
            print("Validation metric: " + str(mean_iou))
            per_batch_iou.append(mean_iou)
    ...
    def _jaccard_index(self, prediction, target, weight, weight_equal):
        """
        Computes IoU for a given target and prediction tensors
        """
        if weight_equal:
            return torch.sum((weight > 0) * (prediction & target)).float() / torch.clamp(torch.sum((weight > 0) * (prediction | target)).float(), min=1e-8)
        return torch.sum(weight * (prediction & target)).float() / torch.clamp(torch.sum(weight * (prediction | target)).float(), min=1e-8)
