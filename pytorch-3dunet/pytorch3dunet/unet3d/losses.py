...
from pytorch3dunet.unet3d.utils import expand_as_one_hot_w_unlabeled
...
class PixelWiseCrossEntropyLoss(nn.Module):
    ...
    def forward(self, input, target, weights):
        ...
        # torch.cuda.memory._dump_snapshot("forward_start.pickle")
        # standard CrossEntropyLoss requires the target to be (NxDxHxW), so we need to expand it to (NxCxDxHxW)
        target = expand_as_one_hot_w_unlabeled(target, C=input.size()[1], unlabeled_index=input.size()[1])
        # torch.cuda.memory._dump_snapshot("after_one_hot.pickle")
        ...
        if self.class_weights is None:
            class_weights = torch.ones(input.size()[1]).float().to(input.device) # TODO (BRIAN) - Should I change this?
        else:
            class_weights = self.class_weights

        # resize class_weights to be broadcastable into the weights
        class_weights = class_weights.view(1, -1, 1, 1, 1) # TODO (BRIAN) - Should I change this?

        torch.cuda.memory._dump_snapshot("/home/brian/notebooks/brian/CellAutoLab_FMMulti/snapshot-preneg-inplace-BASELINE.pickle")

        # torch.cuda.memory._dump_snapshot("after_all_weights.pickle")

        # Identify all the negative labels
        neg_labels_mask = torch.where(target < 0, True, False)

        # torch.cuda.memory._dump_snapshot("after_neg_lab_mask.pickle")
        
        neg_exp = torch.zeros(neg_labels_mask.shape, dtype=torch.int8, device='cuda')
        torch.pow(-1, neg_labels_mask, out=neg_exp)

        


        # normalize the input with negative labels becoming positive before taking the log
        log_probabilities = torch.zeros(neg_labels_mask.shape, dtype=torch.float32, device='cuda')


        log_probabilities += neg_labels_mask
        log_probabilities += (self.softmax(input) * neg_exp)

        eps = torch.finfo(log_probabilities.dtype).tiny # We're getting 0s from softmax for some reason, so this stops that from exploding the log
        log_probabilities += eps

        if torch.any(log_probabilities == 0) or torch.any(torch.isnan(log_probabilities)):
            raise ValueError("pre Log prob zero " + str(torch.any(torch.isinf(log_probabilities))))
        
        log_probabilities = torch.log(log_probabilities)

        if torch.any(torch.isinf(log_probabilities)) or torch.any(torch.isnan(log_probabilities)):
            raise ValueError("Post Log prob inf " + str(torch.any(torch.isinf(log_probabilities))))
        
        # torch.cuda.memory._dump_snapshot("after_log_probs.pickle")

        # multiply weights tensor by class weights
        weights = class_weights * weights

        # compute the losses
        # result = -weights * target * neg_exp * log_probabilities
        weights *= -1
        weights *= target
        # del target
        weights *= neg_exp 
        ...
        if torch.any(torch.isinf(weights)) or torch.any(torch.isnan(weights)):
            raise ValueError("Post Calc prob inf " + str(torch.any(torch.isinf(weights))))

        # with h5py.File("/home/brian/data4/brian/fullyCollapsed/lossTestPathetic.h5", 'w') as f:
        #     f.create_dataset('output', data=input.cpu().detach())
        #     f.create_dataset('target', data=target.cpu().detach())
        #     # f.create_dataset('weights', data=weights.cpu().detach())
        #     f.create_dataset('result', data=weights.cpu().detach())
        #     f.create_dataset('log_probabilities', data=log_probabilities.cpu().detach())
        ...
        # torch.cuda.memory._dump_snapshot("forward_end-checks-AMP-moreinplace.pickle")
