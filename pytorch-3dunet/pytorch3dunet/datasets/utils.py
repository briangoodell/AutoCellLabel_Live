...
def get_test_loaders(config):
    ...
    for test_dataset in sorted(test_datasets, key=lambda ds: ds.file_path, reverse=False):
        logger.info(f'Loading test set from: {test_dataset.file_path}...')
        yield DataLoader(test_dataset, batch_size=batch_size, num_workers=num_workers, collate_fn=prediction_collate, pin_memory=True, shuffle=False)
        # I'll have to rework this anyways for real time, but it feels inefficient making a dataloader for each file
