...
def main():
    ...
    out_folder = "/home/brian/data4/brian/freelyMoving/data/ACLL_unsheared/NRRD_raw"
    warmup_file = "/home/brian/data4/brian/freelyMoving/data/ACLL_unsheared/NRRD_raw/2022-07-27-31_t0001_ch2.nrrd"
    assert os.path.exists(out_folder), "Folder does not exist"
    # assert not os.path.exists(os.path.join(folder, "liveTraces.h5")), "liveTraces.h5 already exists in the folder"

    predictor = _get_predictor(model, None, os.path.join(out_folder, "liveTraces.h5"), config) # We can have just one predictor, because we are saving it all to one file
    ...
    def get_folder(dataset):
        if dataset in datasets_prj_neuropal:
            return os.path.join(input_paths["prj_neuropal"], dataset)
        elif dataset in datasets_prj_rim:
            return os.path.join(input_paths["prj_rim"], dataset)
        elif dataset in datasets_prj_aversion:
            return os.path.join(input_paths["prj_aversion"], dataset)
        elif dataset in datasets_prj_5ht:
            return os.path.join(input_paths["prj_5ht"], dataset)
        elif dataset in datasets_prj_starvation:
            return os.path.join(input_paths["prj_starvation"], dataset)
        else:
    ...
    folders = [os.path.join(get_folder(dataset), "NRRD") for dataset in datasets_test]

    for folder in folders:
        files = [x for x in os.listdir(folder) if x.endswith(".nd2") or x.endswith("ch2.nrrd")]
